...
namespace Modules\Wait\Tests\Http\Controllers\Tests;

use Faker\Provider;
use Illuminate\Http\Response;
use Illuminate\Support\Facades\Artisan;
use Modules\Operation\Database\Seeders\OperationsTableSeeder;
use Modules\Vehicle\Database\Seeders\VehiclesTableSeeder;
use Modules\Wait\Entities\Wait;
use Modules\User\Entities\User;
use Tests\TestCase;
use Illuminate\Foundation\Testing\WithFaker;

class WaitControllerTest extends TestCase
{
    use WithFaker;
    ...
    public function testItCanShowResource()
    {
        ...
        $response = $this->withHeaders([
            'Authorization' => "Bearer {$this->token['superuser']}",
            'Accept' => 'application/json',
        ])->get(route('waits.show', ['wait' => $entity->id]));
    ...
    public function testItCanUpdateResource()
    {
        ...
        $response = $this->withHeaders([
            'Authorization' => "Bearer {$this->token['superuser']}",
            'Accept' => 'application/json',
        ])->put(route('waits.update', ['wait' => $entity->id]), $values);
    ...
    public function testItCanDestroyResource()
    {
        ...
        $response = $this->withHeaders([
            'Authorization' => "Bearer {$this->token['superuser']}",
            'Accept' => 'application/json',
        ])->delete(route('waits.destroy', ['wait' => $entity->id]));
